...
r(2*N+1, 1) = 1 - phi(1) - rs(1);

for jj = 2:N-1; %Using central difference
%     fprintf('jj = %d \t ', jj);
    xi = (jj-1)*delta1; %is actually xi
    zeta=xi*zetac;
    ...
    e11= phic1*rs(jj) - phi(jj)*rc1 - zeta*rc1 - zeta * phi(jj) - phic1 - rs(jj) +...
   ...
   % e_{22} = \left( \Phi  - \zeta  \right) R  \frac{\cos(\zeta)}{\sin(\zeta)} + \frac{1}{2} \left( \left( \Phi  - \zeta  \right) \right)^2 \frac{\cos^2(\zeta)}{\sin^2(\zeta)} +\frac{1}{2} \left(   R^2 - 1  \right) 
    ...
    r(jj, 1) = h * ( ( sigma11*( phi(jj) - zeta - rc1 ) + sigma22s * ( (phi(jj) - zeta)* ( (cos(zeta))^2/(sin(zeta))^2 ) + rs(jj) * ( (cos(zeta))/(sin(zeta) ) )) )* sin(zeta)  - sigma11c1 * ( phic1 + rs(jj) -1 ) * sin(zeta) - sigma11 * ( phic11 + rc1) * sin(zeta) - sigma11 * ( phic1 + rs(jj) -1 ) * cos(zeta) )  - P * ( phi(jj) - zeta) * sqrtA/( (rs(jj))^2 + (phi(jj))^2 + zeta^2 - 2 * phi(jj) * zeta ) ;
    ...
    r(N+jj, 1) =  h * (  ( sigma11 * ( phic1 + rs(jj) - 1 ) + sigma22s * ( rs(jj) + ( phi(jj) - zeta ) * ( cos(zeta) / sin(zeta) ) ) ) * sin(zeta) - sigma11c1 * (rc1 + zeta - phi(jj) ) * sin(zeta) - sigma11 * (rc11 + 1 - phic1)  * sin(zeta) - sigma11 * (rc1 + zeta - phi(jj) ) * cos(zeta) ) - P * R (jj) *sqrtA/( (rs(jj))^2 + (phi(jj))^2 + zeta^2 - 2 * phi(jj) * zeta ) + lm(jj)*( zetac *xi +1 - phi(jj) - rs(jj) ) ;
    
    %equation relating phi and r
    r(2*N+jj, 1) = zetac *xi +1 - phi(jj) - rs(jj);
end
%Boundary Conidtion - continuity
r(N,1) = phi(N)-Phi(1);

r(2*N,1) = dxidzeta1 *  (3*rs(N) - 4*rs(N-1) + rs(N-3))/(2*delta1) -dxidzeta2 *  (-3*R(1) + 4*R(2) - R(3))/(2*delta2) ;

r(3*N, 1) = zetac  +1 - phi(N) - rs(N);
...
%boundary condition, 
r(3*N+1,1) = dxidzeta1 *  (3*phi(N) - 4*phi(N-1) + phi(N-3))/(2*delta1) -dxidzeta2 *  (-3*Phi(1) + 4*Phi(2) - Phi(3))/(2*delta2) ;
r(3*N+I+1,1)=rs(1)-R(1);

for jj = 2:I-1; %Using central difference
%     fprintf('jj = %d \t ', jj);
    xi = (jj-1)*delta2; %is actually xi
    zeta=zetac+(pi/2-zetac)*xi;
