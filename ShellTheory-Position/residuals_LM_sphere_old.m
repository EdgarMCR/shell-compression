...
%     fprintf('jj = %d \t zeta = %.2e \t  e11 = %.2e \t e22s =t %.2e \t sigma11 = %.2e \t sigma11c1 = %.2e \t sigma22s = %.2e  \n',  jj, zeta,  e11, e22s, sigma11, sigma11c1, sigma22s);
    ...
    %Lagraigian Multiplier, forcing r=1
    pContact = rs(jj)- 1;
...
r(2*N,1) = rs(N) - 1; %TODO What condition to impose here?
...
r(2*N+1,1) = dxidzeta1 * (-3*phi(1) + 4*phi(2) - phi(3))/(2*delta1) - dxidzeta2 * (-3*Phi(1) + 4*Phi(2) - Phi(3))/(2*delta2);
