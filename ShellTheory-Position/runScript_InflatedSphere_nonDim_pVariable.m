function[] = runScript_InflatedSphere_nonDim_pVariable(Np, p)
% fprintf('%.4f \n', H);
% Np = str2num(Np)
% p = str2num(p)
global counter;
...
counter=1;
close all;
[X]=initialGuess_inflatedSphere_pVariable(Np);
path='sol.txt';
Y=problem_inflatingSphere_nonDim_pVariable(Np,  p, X, path, 1, path);
fprintf('\n');


[r1] = residuals_inflatingSphere_nonDim_pVariable(X);
[r] = residuals_inflatingSphere_nonDim_pVariable(Y);
