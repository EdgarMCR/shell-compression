function [] = problem_contactRegion(Np, Hh, X, pathToSol,save_data,  savePath)
...
if(xGiven==0)
    ...
    X(2*N+1:3*N, 1) = Data(1:length(Data), 4); 
%     X(3*N +1:3*N +I, 1) = Data(1:length(Data), 2); 
%     X(3*N +I+1:3*N +2*I, 1) = Data(1:length(Data), 3); 
%     X(N+1, 1) = Data(N, 0);
%     X(N+2, 1) = 1;
end

func =@residuals_contactRegion;
