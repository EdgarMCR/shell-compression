function [r] = residuals_inflatingSphere(X)
...
%     fprintf('jj = %d \t ', jj);
    ...
    Rc1=(R(jj+1) -R(jj-1))/(2*delta);
    Rc11=(R(jj+1) - 2*R(jj) +R(jj-1))/delta^2;
    
    Phic1=(Phi(jj+1) -Phi(jj-1))/(2*delta);
    Phic11=(Phi(jj+1) - 2*Phi(jj) +Phi(jj-1))/delta^2;
...
%     fprintf('Phic1 = %.2e \t Rc1 = %.2e \t ', Phic1, Rc1);
   ...
   % e_{22} = \left( \Phi  - \zeta  \right) R  \frac{\cos(\zeta)}{\sin(\zeta)} + \frac{1}{2} \left( \left( \Phi  - \zeta  \right) \right)^2 \frac{\cos^2(\zeta)}{\sin^2(\zeta)} +\frac{1}{2} \left(   R^2 - 1  \right) 
    ...
    %\sigma^{11} = \left( \lambda  + 2 \mu \ \right) e_{11}+  \lambda \frac{1}{\sin^2(\zeta)}  e_{22}
    sigma11 = ( lambda + 2* mu) * e11 + lambda * e22s;
    
    sigma11c1 = ( lambda + 2* mu) * e11c1 + lambda * e22sc1;
    
    %\sigma^{22} = \left[\lambda e_{11} + \left( \lambda  + 2 \mu \ \right) \frac{ e_{22}}{\sin^2(\zeta)} \right]  \ \frac{1}{\sin^2(\zeta)} 
    sigma22s =  lambda * e11 + ( lambda + 2 * mu ) * e22s;
...
r(N,1) = (3*R(N) - 4*R(N-1) + R(N-2))/(2*delta);
