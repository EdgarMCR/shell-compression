function[] = runScript_inflation(Np, p)
% fprintf('%.4f \n', H);
% Np = str2num(Np)
% p = str2num(p)
global H;
...
[X]=initialGuess_inflation(Np);
path='InflatedSphere\\sol_inflated_sphere.txt';

Y=problem_inflation(Np,  p, X, path, 1, path);
fprintf('\n');
count=0;
...
for ii=1:20;
    ...
    fprintf('\n \tii = %d', ii)
    Y=problem_inflation(Np,  p, Y, path, 1, path);
end
